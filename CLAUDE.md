...
Guidance for Claude Code. The canonical shared project guidance lives in `AGENTS.md`; the sections below are duplicated here so Claude Code loads them directly.

## Shared guidance (keep in sync with AGENTS.md)
...
MCP server configuration for Claude Code is in `.mcp.json` at the project root. It exposes the same LangChain documentation servers as the OpenCode setup:

- `docs-langchain` — `https://docs.langchain.com/mcp`
- `reference-langchain` — `https://reference.langchain.com/mcp`

Use these servers for LangChain/LangGraph documentation and API reference queries.
